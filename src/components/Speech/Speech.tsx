...
export const Speech: FC = () => {
  ...
  const nameButton = isListening ? 'Stop' : 'Start';
  return (
    <Container className="Speech">
      <Button className={isListening ? 'stop' : 'start'} ariaLabel={nameButton} onClick={toggleListen}>{nameButton}</Button>
      {/*<Button className="stop" ariaLabel="Stop">Stop</Button>*/}
      <Container className="Transcription">
        {!finalText && !interimText && 'Esperando a que empiece la clase...'}
        {finalText}
        {interimText}
      </Container>
    </Container>
